...
# Calculate variance, mean, and count for each bin
bin_stats = [(np.var(bin, ddof=1), np.mean(bin), len(bin)) for bin in [bin1, bin2, bin3, bin4]]

# Calculate combined variance using the formula
combined_variance = sum((count - 1) * variance + count * (mean - overall_mean) ** 2 
                        for variance, mean, count in bin_stats) / (sum(count for _, _, count in bin_stats) - len(bin_stats))
